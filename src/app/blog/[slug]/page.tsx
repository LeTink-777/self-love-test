...
export default async function BlogPostPage({ params }: { params: Params }) {
  ...
  const others = BLOG_POSTS.filter((item) => item.slug !== post.slug).slice(0, 3);
        ...
        <article className="mx-auto w-full max-w-3xl px-5 py-14">
          ...
          {others.length ? (
            <section className="mt-14">
              <h2 className="font-display text-2xl text-ink">Читать дальше</h2>
              <ul className="mt-6 space-y-3">
                {others.map((item) => (
                  <li key={item.slug}>
                    <Link
                      href={`/blog/${item.slug}`}
                      className="block rounded-xl border border-line bg-card px-5 py-4 text-ink transition-colors hover:border-accent hover:text-accent"
                    >
                      {item.title}
                    </Link>
                  </li>
