...
function unpackAnswers(raw: string | undefined): QuizAnswers {
  if (!raw) return {};
  try {
...
export async function POST(request: Request) {
  ...
  const answers = unpackAnswers(metadata.answers);
  const userEmail = metadata.userEmail || answers.email || "";
  const userName = metadata.userName || answers.name || "Дорогой клиент";
  ...
  try {
    const sections = buildSections({ ...answers, name: userName }, plan);
    ...
    await sendResultEmail({
      to: userEmail,
      subject: `${SITE.productName} — ваш разбор готов`,
      userName,
      resultHtml: sections
        .map(
          (section) =>
            `<h3 style="font-size:16px;margin:22px 0 6px;">${escapeHtml(section.title)}</h3>` +
            `<p style="font-size:14px;line-height:1.65;white-space:pre-line;margin:0;">${escapeHtml(section.content)}</p>`,
        )
        .join(""),
      pdfBuffer,
      fileName: "result.pdf",
