...
/**
 * Answers travel as one JSON metadata value rather than one key each: YooKassa
 * allows 16 metadata keys, and the longer quizzes here have more questions than
 * that once the reserved keys are counted.
 */
function packAnswers(value: unknown): string {
  if (!value || typeof value !== "object" || Array.isArray(value)) return "";
  const out: Record<string, string> = {};
  for (const [key, entry] of Object.entries(value as Record<string, unknown>)) {
    if (key === "email") continue;
    if (typeof entry === "string" && entry) out[key.slice(0, 24)] = entry.slice(0, 120);
  }
  let packed = JSON.stringify(out);
  // Drop free-text fields first if the payload does not fit the 512-char limit.
  if (packed.length > 500) {
    delete out.situation;
    delete out.profession;
    packed = JSON.stringify(out);
  }
  return packed.length > 500 ? packed.slice(0, 500) : packed;
}
...
export async function POST(request: Request) {
  ...
  if (!shopId || !secretKey) {
    console.error("[checkout] YOOKASSA_SHOP_ID / YOOKASSA_SECRET_KEY are not configured");
  ...
  const answers = packAnswers(body.answers);

  const origin = new URL(request.url).origin;
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || origin || SITE.url;
  const returnUrl = `${baseUrl.replace(/\/$/, "")}/thank-you?plan=${plan.id}`;

  // No payment_method_type is sent on purpose: YooKassa then shows every
  // method enabled for the shop — card, SBP, YooMoney, installments.
  const payload: Record<string, unknown> = {
    amount: { value: plan.price.toFixed(2), currency: "RUB" },
    capture: true,
    description: trim128(`${SITE.productName}. ${plan.yooDescription}`),
    confirmation: { type: "redirect", return_url: returnUrl },
    // The webhook reads userEmail/userName/answers back out of here to build
    // and send the result, so anything delivery needs must live on the payment.
    metadata: {
      plan: plan.id,
      plan_name: plan.name,
      site: SITE.domain,
  ...
  if (!response.ok) {
    ...
    console.error("[checkout] YooKassa error", response.status, raw);
    return NextResponse.json(
  ...
  try {
    ...
    console.error("[checkout] unparsable YooKassa response", raw);
    return NextResponse.json({ error: "Не удалось создать платёж." }, { status: 502 });
  }

  const confirmationUrl = payment.confirmation?.confirmation_url;
  if (!confirmationUrl) {
    console.error("[checkout] missing confirmation_url", raw);
    return NextResponse.json({ error: "Не удалось создать платёж." }, { status: 502 });
  }
