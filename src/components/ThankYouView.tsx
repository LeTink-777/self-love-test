...
import { markPaid, readCheckoutSnapshot, type Lead } from "@/lib/lead";
import { PLANS, isPlanId } from "@/lib/plans";
import { SITE } from "@/lib/site";

export function ThankYouView({ planFromUrl }: { planFromUrl: string | null }) {
  const [answers, setAnswers] = useState<Lead>({});
  const [plan, setPlan] = useState<string>(planFromUrl ?? "standard");
  ...
  async function handleDownload() {
    ...
    try {
      const response = await fetch("/api/generate-pdf", {
        ...
        body: JSON.stringify({ userData: answers, plan }),
      });
    ...
    <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col justify-center px-5 py-20">
      <div className="rounded-3xl border border-line bg-card p-8 sm:p-10">
        ...
        <p className="mt-4 leading-relaxed text-muted">
          {selected ? (
            <>
              Тариф «{selected.name}» оформлен. Полный разбор отправлен на
              указанную почту — файл придёт в течение {selected.delivery}.
            </>
          ) : (
            <>
              Полный разбор отправлен на указанную почту в течение срока,
              указанного в выбранном тарифе.
            </>
          )}
        </p>
        ...
        >
          ...
          {downloading ? "Формируем PDF..." : "Скачать PDF с полным разбором"}
        </button>
        ...
        <Link
          href="/result"
          className="mt-4 block text-center text-sm text-accent underline underline-offset-4"
        >
          Открыть полный разбор на сайте
        </Link>

        {selected?.audioUrl ? (
          <a
            href={selected.audioUrl}
            className="mt-6 flex w-full items-center justify-center gap-2.5 rounded-xl border border-line px-6 py-3.5 text-sm font-semibold text-ink transition-colors hover:border-accent hover:text-accent"
          >
            <Headphones className="size-4" aria-hidden="true" />
            Аудиокомментарий к разбору
          </a>
        ) : null}

        {selected?.calendlyUrl ? (
          <a
            href={selected.calendlyUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-3 flex w-full items-center justify-center gap-2.5 rounded-xl border border-line px-6 py-3.5 text-sm font-semibold text-ink transition-colors hover:border-accent hover:text-accent"
          >
            <CalendarCheck className="size-4" aria-hidden="true" />
            Записаться на личную сессию 30 минут
          </a>
        ) : null}
